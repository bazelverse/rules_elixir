...
load("//:ex_unit_test.bzl", "ex_unit_test")
...
def ex_unit_tests(
    ...
    Args:
      ...
      config_loader: main-repo label of the .exs that does that evaluation. Required when
        load_config is set. A macro cannot resolve a Label to a runfiles path, so the path
        is derived from the label text and an external repo cannot be named here.
    ...
    if load_config:
        if not config_loader:
            fail("load_config = True requires config_loader, the label of the .exs that " +
                 "evaluates config/config.exs before test_helper.exs runs")
        if not config_loader.startswith("//"):
            fail("config_loader must be a main-repo label (//pkg:file.exs); this is a macro, " +
                 "so the runfiles path is derived from the label text and an external repo's " +
                 "canonical name is not knowable here. Got: " + config_loader)
        depth = len(native.package_name().split("/")) if native.package_name() else 0
        loader = ("../" * depth) + config_loader[2:].replace(":", "/")
        ...
        extra_data = extra_data + [config_loader] + native.glob(
            ["config/**"],
            allow_empty = True,
        )
